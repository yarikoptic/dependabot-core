  ...
  module Python
    class FileFetcher < Dependabot::FileFetchers::Base
      ...
      def project_files
        ...
        path_dependencies.each do |dep|
          ...
          unfetchable_deps << if sdist_or_wheel?(path)
                                ...
                                manifest = cleanpath(File.join(directory, dep[:file]))
                                fullpath = cleanpath(File.join(directory, path))

                                "`#{dep[:name]}` at #{manifest} (no setup.py or pyproject.toml found in #{fullpath})"
                              end
        end
      ...
      def parse_requirement_path_dependencies(req_file)
        # If this is a pip-compile lockfile, rely and whatever path dependencies we found in the main manifest
